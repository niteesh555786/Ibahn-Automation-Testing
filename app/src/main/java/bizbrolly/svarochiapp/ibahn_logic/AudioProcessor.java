...
public class AudioProcessor extends Thread {
	...
	public static final int MAXOVER_MSG = 2;
	...
	public AudioProcessor(Handler handle, Context context) {
		this.mHandle = handle;
	...
	public void run() {
		try {
			...
			while (this.mIsRunning) {
				...
				if (!mShowMaxValue) {
					Message msg = mHandle.obtainMessage(DATA_MSG, splValue);
					mHandle.sendMessage(msg);
				} else {
					Message msg = mHandle.obtainMessage(DATA_MSG, mMaxValue);
					mHandle.sendMessage(msg);
					Thread.sleep(2000);
