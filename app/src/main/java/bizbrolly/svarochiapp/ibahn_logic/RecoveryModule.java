...
public class RecoveryModule {
    private final int TIMEOUT = 3333;
    private String currentData;
    private boolean waitingForData;

    public void fetchState(final int type, final int deviceId, final Callback callback) {
    ...
    private void beginTimeout(final Callback callback) {
//        if (runnable == null) {
//            handler = new Handler();
...
//        handler.postDelayed(runnable, TIMEOUT);
    ...
    private void cancleTimeOut() {
        handler.removeCallbacks(runnable);
    }

    private void basicTimeOut(final Callback callback) {
        if (runnable == null) {
            handler = new Handler();
            runnable = new Runnable() {
                @Override
    ...
    private interface Data {
        final String POWER = "L0";
        final String INTENSITY_LEVEL = "L1";
        final String TUNABLE_LEVEL = "L2";
    }
}
